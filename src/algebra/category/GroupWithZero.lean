...
@[priority 100]
instance mul_equiv_class.to_monoid_with_zero_hom_class {F α β : Type*} [mul_zero_one_class α]
  [mul_zero_one_class β] [mul_equiv_class F α β] : monoid_with_zero_hom_class F α β :=
{ coe := coe_fn,
  coe_injective' := fun_like.coe_injective,
  map_one := sorry,
  map_zero := sorry,
  ..‹mul_equiv_class F α β› }
...
/-- The category of group_with_zeros. -/
...
instance has_forget_to_Pointed : has_forget₂ GroupWithZero Pointed :=
{ forget₂ := { obj := λ X, ⟨X, 0⟩, map := λ X Y f, ⟨f, f.map_zero'⟩ } }
