...
begin
  ...
  replace hd : differentiable_on ℂ (dslope f c) (ball c R₁),
    from (differentiable_on_dslope $ ball_mem_nhds _ hR₁).mpr hd,
  refine norm_le_of_forall_mem_frontier_norm_le (is_compact_closed_ball c r)
    (hd.mono $ closed_ball_subset_ball hr.2) _ hr.1.le,
  rw frontier_closed_ball',
  intros z hz,
  have hz' : z ≠ c, by { rintro rfl, simpa [hr₀.ne] using hz },
  rw [dslope_of_ne _ hz', slope_def_field, normed_field.norm_div,
